...
export function BudgetIncidentCard({
  ...
  return (
    <Card className="overflow-hidden border-red-500/25 bg-red-500/[0.06]">
      <CardHeader className="px-5 pt-5 pb-3">
        <div className="flex items-start justify-between gap-3">
          <div>
            <div className="text-[11px] uppercase tracking-[0.18em] text-red-700/80 dark:text-red-200/80">
            ...
            <CardDescription className="mt-1 text-red-700/80 dark:text-red-100/70">
              Spending reached {formatCents(incident.amountObserved)} against a limit of {formatCents(incident.amountLimit)}.
            </CardDescription>
          </div>
          <div className="rounded-full border border-red-500/30 bg-red-500/10 p-2 text-red-700 dark:text-red-200">
            <AlertOctagon className="h-4 w-4" />
          </div>
        </div>
      </CardHeader>
      <CardContent className="space-y-4 px-5 pb-5 pt-0">
        <div className="flex items-start gap-2 rounded-lg border border-red-500/25 bg-red-500/10 px-3 py-2 text-sm text-red-800 dark:text-red-100/90">
