  ...
  return (
    <Card>
      <CardHeader className="px-4 pt-4 pb-1">
        <CardTitle className="text-base">Billing entry mix</CardTitle>
        <CardDescription>Account-level charges grouped by type.</CardDescription>
      </CardHeader>
      <CardContent className="space-y-2 px-4 pb-4 pt-3">
        {rows.length === 0 ? (
          <p className="text-sm text-muted-foreground">No billing entries in this period.</p>
        ) : (
          rows.map((row) => (
            <div
              key={row.eventKind}
            ...
            >
              <div className="min-w-0">
                ...
                <div className="text-xs text-muted-foreground">
                  {row.eventCount} entr{row.eventCount === 1 ? "y" : "ies"} · {row.billerCount} vendor{row.billerCount === 1 ? "" : "s"}
                </div>
              </div>
              <div className="text-right tabular-nums">
                <div className="text-sm font-medium">{formatCents(row.netCents)}</div>
                <div className="text-xs text-muted-foreground">
                  {formatCents(row.debitCents)} charges
                </div>
              </div>
            </div>
          ))
